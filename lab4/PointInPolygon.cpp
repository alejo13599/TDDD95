...
while(NumberOfVertices){
    ...
    for(int j=0; j<NumberOfPoints; j++){
        ...
        for(int i=0; i <NumberOfVertices; i++){
            //cout << NumberOfPoints << " " << NumberOfVertices << "hej";
            int direction1 = checkdirection(Pointvec[i], Pointvec[i+1], testpoint);
            int direction2 = checkdirection(Pointvec[i], Pointvec[i+1], {10001, 10001});
            int direction3 = checkdirection(testpoint, {10001, 10001}, Pointvec[i]);
            int direction4 = checkdirection(testpoint, {10001, 10001}, Pointvec[i+1]);
        ...
        int direction2 = checkdirection(Pointvec[NumberOfVertices-1], Pointvec[0], {10001, 10001});
        int direction3 = checkdirection(testpoint, {10001, 10001}, Pointvec[NumberOfVertices-1]);
        int direction4 = checkdirection(testpoint, {10001, 10001}, Pointvec[0]);
        if(direction1 != direction2 && direction3 != direction4){
            intersections += 1;
        }
